+++
title = "Publication"
+++

## Publications

### 2025

- **Slot: Provenance-Driven APT Detection through Graph Reinforcement Learning.**  
  <u>Wei Qiao</u>, Yebo Feng, Teng Li, Zhuo Ma, Yulong Shen, JianFeng Ma, Yang Liu.  
  *Proceedings of the 2025 ACM SIGSAC Conference on Computer and Communications Security (CCS)*, 2025, pp. 963-977.  

- **SauronEyes: Disentangling Voluminous Logs to Unveil Camouflaged Attack Intentions.**  
  <u>Wei Qiao</u>, Weiheng Wu, Song Liu, Teng Li, Yebo Feng, Zehui Wang, Junrong Liu, Bo Jiang, Zhigang Lu, Baoxu Liu.  
  *IEEE Transactions on Information Forensics and Security (TIFS)*, 2025.  

- **Chaos: Robust Spatio-Temporal Fusion for Generalizable APT Provenance Tracing.**  
  Teng Li, <u>Wei Qiao<sup>(*)</sup></u>, Yebo Feng, Jiahua Xu, Paolo Tasca, Weiguo Lin, Zexu Dang, Zhuo Ma, Jianfeng Ma.  
  *International Conference on Data Security and Privacy Protection (DSPP)*, 2025, pp. 507-524. (Best Paper Award)

- **Brewing Vodka: Distilling Pure Knowledge for Lightweight Threat Detection in Audit Logs.**  
  Weiheng Wu, <u>Wei Qiao</u>, Wenhao Yan, Bo Jiang, Yuling Liu, Baoxu Liu, Zhigang Lu, Junrong Liu.  
  *Proceedings of the ACM on Web Conference 2025 (WWW)*, 2025, pp. 2172-2182.

- **Sentient: Multi-Scenario Behavioral Intent Analysis for Advanced Persistent Threat Detection.**  
  Wenhao Yan, Ning An, <u>Wei Qiao</u>, Weiheng Wu, Bo Jiang, Yuling Liu, Zhigang Lu, Junrong Liu.  
  *AAAI Conference on Artificial Intelligence (AAAI)*, 2025.

- **ProvX: Generating Counterfactual-Driven Attack Explanations for Provenance-Based Detection.**  
  Weiheng Wu, <u>Wei Qiao</u>, Teng Li, Yebo Feng, Zhuo Ma, Jianfeng Ma, Yang Liu.  
  *arXiv preprint arXiv:2508.06073*, 2025.

- **Towards Effective Black-Box Attacks on DoH Tunnel Detection Systems.**  
  Linghao Li, Yan Zhu, Yun Li, <u>Wei Qiao</u>, Zelin Cui, Susu Cui, Bo Jiang, Zhigang Lu.  
  *Computer Networks*, 2025, pp. 111524.

- **PathWatcher: A Path-Based Behavior Detection Method for Attack Detection and Investigation.**  
  Zehui Wang, Hao Li, Yinhao Qi, <u>Wei Qiao</u>, Song Liu, Chen Zhang, Bo Jiang, Zhigang Lu.  
  *Computers & Security*, 2025, pp. 104563.

- **PanThreat: Global Resource-Based Anomaly Detection for APTs.**  
  Wenhao Yan, Weiheng Wu, Bingsheng Bi, <u>Wei Qiao</u>, Bo Jiang, Yuling Liu, Junrong Liu.  
  *2025 28th International Conference on Computer Supported Cooperative Work in Design (CSCWD)*, 2025, pp. 135-140.

- **DCASI: A Sequence-based Attack Investigation Method Using DTW Contrastive Learning.**  
  Yun Li, <u>Wei Qiao</u>, Yan Zhu, Yunxiang Wang, Bo Jiang, Zhigang Lu.  
  *2025 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP)*, 2025, pp. 1-5.

### 2023

- **T-trace: Constructing the APTs Provenance Graphs through Multiple Syslogs Correlation.**  
  Teng Li, Ximeng Liu, <u>Wei Qiao</u>, Xiongjie Zhu, Yulong Shen, Jianfeng Ma.  
  *IEEE Transactions on Dependable and Secure Computing (TDSC)*, 2023, vol. 21, no. 3, pp. 1179-1195.
